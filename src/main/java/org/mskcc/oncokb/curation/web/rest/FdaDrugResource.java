...
public class FdaDrugResource {
    ...
    /**
     * {@code SEARCH  /_search/fda-drugs?query=:query} : search for the fdaDrug corresponding
     * to the query.
     *
    ...
    public ResponseEntity<List<FdaDrug>> searchFdaDrugs(@RequestParam String query, Pageable pageable) {
        ...
        Page<FdaDrug> page = fdaDrugService.search(query, pageable);
        HttpHeaders headers = PaginationUtil.generatePaginationHttpHeaders(ServletUriComponentsBuilder.fromCurrentRequest(), page);
        return ResponseEntity.ok().headers(headers).body(page.getContent());
    }
}
